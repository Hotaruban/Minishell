...
/*   Updated: 2024/01/23 14:25:26 by jhurpy           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
// 			if (i == ptr_in)
// 				cmd->fd_infile = open(cmd->files[i], O_RDONLY)
			
// 		}
// 		else if (cmd->type[i] == e_heredoc)
// 		{
// 			if (i == ptr_in)
// 				cmd->fd_infile = // heredoc command
// 		}	
// 		else if(cmd->type[i] == e_outfile | cmd->type[i] == e_append)
// 		{
// 			open_outfile(cmd->files[i], cmd->type[i], cmd);
// 			if (i == ptr_out)
// 				cmd->fd_outfile = open(cmd->files[i], O_WRONLY);
// 		}
...
// 		i++;
// 	}
// 	if (ptr_in >= 0) //&& cmd->cmd[0] != NULL)
// 	{
// 		if (cmd->type[ptr_in] == e_heredoc)
// 			cmd->fd_infile = cmd->here_doc_fd; 				/// implement function heredoc ?
// 		else
// 			cmd->fd_infile = open(cmd->files[ptr_in], O_RDONLY);
// 	}
// 	if (ptr_out >= 0) // && cmd->cmd[0] != NULL)
// 	{
